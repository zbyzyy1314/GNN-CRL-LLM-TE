...
# ─── 2. Link Failures ───
print(f'\n{"="*60}')
print(f'2. Link Failures (LMTE Fig 10a)')
print(f'{"="*60}')
for num_failures in [1, 3, 5, 10]:
    topo = Topology('data/GEANT', max_paths_per_pair=8)
    traffic = TrafficLoader('data/GEANTTM2', topo.num_nodes)
    env = TEEnv(topo, traffic, device='cuda')
    # Reduce random links to 10% capacity
...
print(f'\n{"="*60}')
print(f'3. Traffic Bursts (LMTE Fig 11)')
print(f'{"="*60}')
for scale in [2, 5, 10, 20, 30]:
    topo = Topology('data/GEANT', max_paths_per_pair=8)
    traffic = TrafficLoader('data/GEANTTM2', topo.num_nodes)
    # Burst: add scaled zero-mean Gaussian noise to each TM
    orig_real = traffic.get_real_traffic(normalize=False)
    rng = np.random.RandomState(42)
    burst_tms = []
    for tm in orig_real:
        noise_scale = tm.std() * scale / 10.0  # normalized burst
        noise = rng.randn(*tm.shape) * noise_scale
        np.fill_diagonal(noise, 0)
        burst_tms.append(tm + noise)
    traffic.real_tm = np.stack(burst_tms)
    env = TEEnv(topo, traffic, device='cuda')
    env.precompute_ecmp()
    policy = TEGNNPolicy(topo, hidden_dim=128, max_k=topo.max_k).cuda()
    value = GNNValueNetwork(topo, hidden_dim=128).cuda()
...
print(f'\n{"="*60}')
print(f'4. Natural Drift (LMTE Table 1 style)')
print(f'{"="*60}')
traffic_full = TrafficLoader('data/GEANTTM', topo.num_nodes)
total_tms = traffic_full.num_tms
for start_pct, label in [(0, '0-25%'), (25, '25-50%'), (50, '50-75%')]:
    start = int(total_tms * start_pct / 100)
    end = int(total_tms * (start_pct + 25) / 100)
    tms_slice = traffic_full.get_real_traffic(normalize=False)[start:end]
    # Create a TrafficLoader-like object with just this slice
    class SliceLoader:
