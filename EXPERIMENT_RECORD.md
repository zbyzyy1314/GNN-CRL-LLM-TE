...
LMTE GEANT (来自 LMTE GitHub): 22 节点, 72 链路, 462 SD 对, 10773 个 TM (bps)

我们的格式: 已转换为 kbps (bps/1000), 路径 8 条/对

### LP Optimal (流约束, 全 2152 测试 TM, ~21 分钟)
...
### LMTE GEANT 上的方法对比 (8-最短路径, ECMP=22.38)

| 方法 | MLU | vs ECMP | 参数量 |
|---|---|---|---|
| LP Optimal | 1.88 | 91.6% | — |
| LMTE (LLaMA-7B, mlu.txt) | 1.58 | 92.9% | 7,000,000,000 |
| GNN+LLM (Qwen-1.5B, 20ep) | 2.24 | 90.0% | 1,500,000,000 |
| 纯 GNN (220K, 40ep) | 2.47 | 89.0% | 220,000 |

### GNN+LLM 鲁棒性 vs LMTE (LMTE GEANT 数据)

```
测试项目           GNN+LLM    LMTE 论文
链路故障 1条:      +21%       <10%
链路故障 3条:      +7%        <10%
流量突发 2×:       +0%        <5%
流量突发 5×:       +2%        <5%
...
*LMTE 链路故障鲁棒性来自专用启发式算法(非模型)。*

### 跨拓扑泛化

| 方法 | Abilene MLU | 说明 |
|---|---|---|
| Abilene 正常训练 (上界) | 0.087 | — |
| GNN 转移 (GEANT→Abilene) | 0.319 | 仅 MPBlock 层转移 |
| LLM 零样本 (GEANT→Abilene) | 0.269 | LLM 有一定泛化但远不如重训 |

---

## 七、LMTE CMDP 约束对比实验（2024-07-27）

### 实验配置

- 数据集: LMTE GEANT (22节点, 72链路, 462 SD对)
- 训练: 前80%时段 (8618 TM), 测试: 后20%时段 (2155 TM)
- 约束阈值: mean_util ≤ 0.25, overload ≤ 0.1, p95 ≤ 1.5
- ECMP: MLU=22.38, util=0.69, overload=0.31, p95=2.00
- LP Optimal (测试集2152 TM): MLU=1.88±0.45 [0.89, 7.01]
- LP无法满足约束: 仅0.8% TM可MLU<1.0, 仅20.6%可MLU<1.5 → 阈值对LP也不可达

### 非时序结果 (Seed 42)

| Method | MLU | mean_util | overload | p95 | Σ违规↓ | vs baseline MLU | vs baseline Σ违规 |
|---|---|---|---|---|---|---|---|
| LP Optimal* | 1.88 | ≤MLU | ≤MLU | ≤MLU | ≤3.79 | — | — |
| ECMP | 22.38 | 0.690 | 0.315 | 2.000 | 1.155 | — | — |
| **baseline GNN** | **2.04** | 0.368 | 0.150 | 1.744 | 0.412 | — | — |
| combined CNN | 2.11 | 0.327 | 0.113 | 1.735 | 0.325 | -3.5% | **-21.1%** |
| lagrangian GNN | 2.20 | 0.306 | 0.113 | 1.777 | 0.346 | -8.0% | -16.0% |
| combined GNN | 2.24 | 0.301 | 0.114 | 1.785 | 0.350 | -9.7% | -15.0% |
| cvar GNN | 2.47 | **0.292** | **0.111** | 1.800 | 0.353 | -21.0% | -14.3% |

> *LP只提供MLU最优值; util/overload/p95为MLU上界估计
> Σ违规 = max(0, util-0.25) + max(0, overload-0.1) + max(0, p95-1.5)

**分析:**
1. **Baseline MLU最优 (2.04) 但约束违规最严重** — 无约束, 纯优化MLU
2. **Combined CNN 综合最优** — MLU=2.11 (仅差3.5%), Σ违规减少21%
3. **Lagrangian 效果居中** — MLU和约束的平衡, 未因λ爆炸而崩溃(seed42)
4. **CVaR 约束最好但MLU最差** — mean_util最低(0.292), 但MLU恶化21%
5. **所有方法违反所有约束** — 因为LP都无法满足0.25/0.1/1.5, 阈值对数据集不可达

### 时序结果 (GNN, 3 seeds × 4 methods)

| Method | MLU (mean) | ±std | 稳定性 | vs 非时序 |
|---|---|---|---|---|
| **baseline** | **2.09** | 0.03 | ✅ 稳定 | +2.4% |
| cvar | 2.18 | 0.01 | ✅ 稳定 | -11.7% |
| lagrangian | 2.58 | **0.32** | ❌ 崩溃 | +17.2% |
| combined | 2.78 | 0.10 | ❌ 崩溃 | +23.9% |

**分析:**
1. **Baseline/CVaR 时序稳定** — 12步历史窗口对baseline和cvar无害
2. **Lagrangian/Combined 在2/3种子下崩溃** — λ在时序模式下不稳定, MLU暴涨至2.8+
3. **时序 vs 非时序: baseline几乎无差异 (2.04 vs 2.09)** — 历史窗口加成有限
4. **CVaR时序比非时序更好 (2.18 vs 2.47)** — 时序历史帮助尾部优化

### 关键发现

1. **约束阈值 0.25/0.1/1.5 对 LMTE 数据不可达** — LP最优也违反, λ必然发散
2. **CNN 略优于 GNN (非时序)** — 2.11 vs 2.24 (combined), 但差距不大
3. **Lagrangian 有价值但需要合理阈值** — seed42下MLU=2.20仅差baseline 8%, 且显著降低约束违规
4. **时序对基本方法无负面影响** — baseline/CVaR时序和非时序MLU接近
5. **建议: 用LP可达阈值重跑** — mean_util用LP的MLU×0.5作为上界, 或直接和目标方法比较
|---|---|---|
| Abilene 正常训练 (上界) | 0.087 | — |
| GNN 转移 (GEANT→Abilene) | 0.319 | 仅 MPBlock 层转移 |
| LLM 零样本 (GEANT→Abilene) | 0.269 | LLM 有一定泛化但远不如重训 |
...
LLM 贡献有限的原因:
1. Qwen-1.5B < LLaMA-7B (模型大小差距)
2. 线性投影+LoRA < 交叉注意力 (对齐方式)
3. Conv1d < RNN (时序编码)
4. TE 任务中 GNN 本身已足够强 (220K 参数接近最优)

### 最终结论

1. **220K GNN 已捕获 89% 的 ECMP→LP 改进空间**——大多数场景下 LLM 加成有限
2. **CMDP 约束机制在重载数据上才有效**——轻载时 λ=0, 无差异
3. **时序因果评估验证了方法的实用性**——用历史 TM 决策当前路由, MLU 未退化
4. **跨拓扑泛化是结构性难题**——简单拼接无法实现, 需要重新设计架构
```
